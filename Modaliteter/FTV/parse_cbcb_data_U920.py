...
def keep_only_axial_images(series: CtSeries, alignment_limit=0.9) -> CtSeries:
    """As some dental CBCT, e.g., Morita Accuitomo 170, apparently give different reconstructions the same
    SeriesInstanceUID, this function is required to remove all images that are not axial
    :param series: A DICOM CT series as a dicom-image-tools CtSeries object instance
    :return: The DICOM CT series with all but the axial images removed
    """

    alignments = []

    for dcm_file in series.CompleteMetadata:
        alignments.append(check_cbct_stack_axial_alignment(dcm_file, mode='calc_alignment'))

    keep_indices = np.asarray(alignments) > alignment_limit
    keep_indices = keep_indices.tolist()

    series.CompleteMetadata = [
        metadata
        for ind, metadata in enumerate(series.CompleteMetadata)
        if ind in keep_indices
    ]

    series.VoxelData = [
        voxel_data
        for ind, voxel_data in enumerate(series.VoxelData)
        if ind in keep_indices
    ]

    series.FilePaths = [
        fp for ind, fp in enumerate(series.FilePaths) if ind in keep_indices
    ]

    series.SlicePosition = [
        sp for ind, sp in enumerate(series.SlicePosition) if ind in keep_indices
    ]
...
for pat in folder.iterdir():
    ...
    data[selected_study].Series = [
        d for (d, remove) in zip(data[selected_study].Series, idx_rem) if not remove]
