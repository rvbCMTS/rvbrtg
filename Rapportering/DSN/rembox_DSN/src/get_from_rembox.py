...
def get_study_and_series_data():
    ...
    # Hämtar CT-data
    study_data, series_data = get_data_from_ct(rembox=rembox)
...
def get_data_from_ct(rembox: REMboxDataQuery) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def get_study_data_dictionary(study_data):
...
def get_study_data_report(study_data_dict):
    ...
    for patient_group in study_data_dict:
        ...
        count_criteria = get_count_criteria(patient_group=patient_group)

        # Hämta relevanta undersökningskoder för patientgruppen
        procedure_codes = get_procedure_codes_dict(patient_group)
...
def get_count_criteria(patient_group):
...
def get_procedure_codes_dict(patient_group):
    if patient_group == "adults":
