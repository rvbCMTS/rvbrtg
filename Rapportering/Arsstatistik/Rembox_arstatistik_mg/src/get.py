...
import pandas as pd
import datetime
import Rapportering.Arsstatistik.Rembox_arstatistik_mg.src.constants as con
...
def get_study_and_series_data(year):
    ...
    # Hämtar CT-data
    study_data, series_data = get_data_from_mg(rembox=rembox, year=year)

    return study_data, series_data


def get_data_from_mg(rembox: REMboxDataQuery, year) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def get_report_df():
    report_df = pd.read_excel('src/MAM Mall årsredovisning DosReg.xlsx', header=[0, 1])
...
def get_studies_and_agd(report_df, study_data, patient_group_studies, patient_group_agd):
    for procedure in con.procedure_codes_dict:
        ...
        mean_agd = study_data_filtered["accumulatedAverageGlandularDoseBothBreasts"].mean()

        report_df.loc[
            report_df["Undersökningskod(er)"] == code_as_str, patient_group_studies
        ] = number_of_studies
        report_df.loc[
            report_df["Undersökningskod(er)"] == code_as_str, patient_group_agd
        ] = mean_agd
...
def get_study_data_dict(study_data):
    ...
    study_data_male = study_data[study_data['patientsSex'] == 'M']
    study_data_female = study_data[study_data['patientsSex'] == 'F']

    dict_with_study_data = {
       # 'girl': study_data_kids_female,
       # 'boy': study_data_kids_male,
        'male': study_data_male,
        'female': study_data_female
    }
...
def get_report_dict(study_data_dictionary, report_dataframe):
    ...
    for hospital in con.machines_at_hospital:
        ...
        for patient_group in study_data_dictionary:
            ...
            report_dataframe = get_studies_and_agd(
                ...
                patient_group_agd=con.patient_group['{}-agd'.format(patient_group)]
            )
