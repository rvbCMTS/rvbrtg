...
from Rapportering.Arsstatistik.Rembox_arstatistik_gml.src.constants import (
    MACHINES_AT_HOSPITALS,
)


def get_study_and_series_data(year: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    rembox_local = REMboxDataQuery(
    ...
    # Hämtar int-data
    study_data, series_data = get_data_from_gml(rembox=rembox_local, year=year)

    return study_data, series_data


def get_data_from_gml(year: int, rembox: REMboxDataQuery) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def get_studies_and_dap(report_df, study_data, patient_group_studies, patient_group_dap):
    for procedure in con.procedure_codes_dict:
        ...
        report_df.loc[report_df["Undersökningskod(er)"] == code_as_str, patient_group_dap] = mean_dap
...
def get_report_dict(study_data_dictionary, report_dataframe):
    ...
    for hospital in MACHINES_AT_HOSPITALS:
        print(hospital)
        ...
        for patient_group in study_data_dictionary:
            study_data_at_hospital = study_data_dictionary[patient_group][
                study_data_dictionary[patient_group]["machine"].isin(con.MACHINES_AT_HOSPITALS[hospital])
            ]
