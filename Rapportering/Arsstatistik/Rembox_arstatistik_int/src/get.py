...
def get_report_df():
    report_df = pd.read_excel('src/INT Mall årsredovisning DosReg.xlsx', header=[0, 1]) #Använd rätt mall
...
def get_performing_physician(study_data): # Vad används detta till?
...
def get_studies_and_dap(report_df, study_data, patient_group_studies, patient_group_dlp): #Var pekar de två sista?
    for procedure in con.procedure_codes_dict:
        ...
        mean_dap = study_data_filtered["doseareaproducttotal"].mean() #Kolla så detta är rätt sträng!!

        report_df.loc[
            report_df["Undersökningskod(er)"] == code_as_str, patient_group_studies
        ] = number_of_studies
        report_df.loc[
            report_df["Undersökningskod(er)"] == code_as_str, patient_group_dlp #Var pekar denna?
        ] = mean_dap
    ...
    for hospital in con.machines_at_hospital:
        ...
        for patient_group in study_data_dictionary:
            ...
            report_dataframe = get_studies_and_dap(
                ...
                patient_group_dlp=con.patient_group['{}-dlp'.format(patient_group)] #Var pekar denna??
            )
