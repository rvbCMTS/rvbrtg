...
def strip_scouts_from_series(series):

    for serie in series:
        idx_keeps = find_scout_strip_indices(serie)

        fps = np.asarray(serie.FilePaths)
        serie.FilePaths = fps[idx_keeps].tolist()

    return series


def find_scout_strip_indices(serie):
    a = 1
    idx_keeps = []
...
for pat in folder.iterdir():
    ...
    data[selected_study].Series = strip_scouts_from_series(data[selected_study].Series)
