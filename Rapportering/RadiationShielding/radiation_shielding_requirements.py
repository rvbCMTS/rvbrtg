...
CALCULATION_SITES: list[CalculationSite] = [
    CalculationSite(
        ...
        datum="2023-01-25",
        images_per_year=300,
        distance_tube_to_barrier=1.58
    ),
    CalculationSite(
        site="Bergvattengården",
        datum="2023-03-03",
        distance_tube_to_barrier=1.6
    ),
    CalculationSite(
        site="Nordmaling",
        datum="2023-09-22",
        distance_tube_to_barrier=1.8
    ),
    CalculationSite(
        site="Vännäs",
        datum="2023-11-06",
        images_per_year=2110,
        distance_tube_to_barrier=3.35
    ),
]
...
def calculate_barrier_material_thickness_requirement(calc_site: CalculationSite) -> tuple[str, str, str, str, str, str]:
    need_gypsum = wall_thickness_requirement(
    ...
    need_concrete = wall_thickness_requirement(
        tube_voltage=calc_site.tube_voltage,
        tube_current=calc_site.tube_current,
    ...
    need_lead = wall_thickness_requirement(
        tube_voltage=calc_site.tube_voltage,
        tube_current=calc_site.tube_current,
    ...
    return calc_site.site, calc_site.datum, f"{need_gypsum:.2f}", f"{need_concrete:.2f}", f"{need_lead:.2f}", str(calc_site.images_per_year)
